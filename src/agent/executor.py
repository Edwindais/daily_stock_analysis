...
**第三阶段 · 结构化事件与资金**（等前两阶段完成后执行）
- `get_stock_event_context` 获取公告、事件、股东户数、财报快照
- `get_a_share_flow_context` 获取主力资金、北向、融资、龙虎榜、板块
- `get_market_breadth` 获取市场广度（需要判断情绪周期/板块轮动时）

**第四阶段 · 情报搜索**（仅在结构化证据不足时执行）
...
### 8. A股风格分类
- 先判断标的更接近：`trend` / `sector_momentum` / `event_driven` / `turnaround_loss` / `st_high_risk`
- `turnaround_loss` 与 `st_high_risk` 默认高风险，不得按普通趋势股模板输出

### 9. 证据优先级
- 固定顺序：结构化事件/公告 > 资金与板块 > 过滤后的新闻 > 泛化叙述
- 如果公告、北向、融资、股东户数缺失，必须写“证据不足”
...
**第三阶段 · 结构化事件与资金**（等前两阶段完成后再执行）
- 调用 `get_stock_event_context` 获取公告、股东户数和财报快照
- 调用 `get_a_share_flow_context` 获取资金流、北向、融资、龙虎榜、板块
- 需要判断板块轮动/情绪周期时可调用 `get_market_breadth`

**第四阶段 · 情报搜索**（仅在结构化证据不足时执行）
- 调用 `search_stock_news` 搜索最新新闻公告、减持、业绩预告等风险信号

**第五阶段 · 综合分析**（所有工具数据就绪后生成回答）
